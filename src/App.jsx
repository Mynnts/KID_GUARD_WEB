...
const PARENT_STEPS = [
  { icon: '👤', title: 'สมัครสมาชิก', desc: 'เลือก "ผู้ปกครอง" → ลงทะเบียนด้วยอีเมลและรหัสผ่าน → ระบบจะสร้าง PIN 6 หลักให้อัตโนมัติ' },
  { icon: '👶', title: 'เพิ่มโปรไฟล์เด็ก', desc: 'กดปุ่ม "เพิ่มเด็ก" → เลือก Avatar → กรอกชื่อเล่นและอายุ → ตั้งเวลาจำกัดต่อวัน → เลือกโหมดดูแล' },
  { icon: '📍', title: 'ติดตามตำแหน่ง', desc: 'ดูตำแหน่งเรียลไทม์ของเด็กบนแผนที่ อัปเดตอัตโนมัติ' },
  { icon: '⏰', title: 'จัดการเวลาหน้าจอ', desc: 'ตั้งเวลาจำกัดต่อวัน ดูเวลาที่ใช้ไปและเวลาที่เหลือ สามารถหยุดชั่วคราวได้ทันที' },
  { icon: '📊', title: 'ควบคุมแอป', desc: 'ดูรายการแอปทั้งหมด บล็อกแอปไม่เหมาะสม ค้นหาและรีเฟรชรายการแอปล่าสุด' },
  { icon: '📅', title: 'จัดตารางเวลา', desc: 'ตั้งเวลานอนหลับ (Sleep Schedule) และช่วงเวลาสงบ (Quiet Time) เช่น เวลาทำการบ้าน' },
  { icon: '🌟', title: 'ระบบรางวัล', desc: 'สร้างรางวัลพร้อม Emoji และแต้มที่ต้องสะสม แก้ไข/ลบรางวัลได้ด้วยการกดค้าง' },
];

const CHILD_STEPS = [
  { icon: '🔗', title: 'เชื่อมต่อกับผู้ปกครอง', desc: 'เปิดแอป → เลือก "เด็ก" → กรอก PIN 6 หลักจากผู้ปกครอง → ระบบเชื่อมต่ออัตโนมัติ' },
  { icon: '😄', title: 'สร้างโปรไฟล์', desc: 'เลือก Avatar ที่ชอบ → กรอกชื่อเล่นและอายุ → กด "เริ่มต้นใช้งาน"' },
  { icon: '🛡️', title: 'เปิดโหมดเด็ก', desc: 'กดเปิดโหมดเด็ก → ระบบนับเวลา/บล็อกแอป/ตารางเวลาทำงานอัตโนมัติ → ปิดได้เฉพาะ PIN ผู้ปกครอง' },
  { icon: '⭐', title: 'ดูแต้มสะสม', desc: 'ดูแต้มที่มีและรางวัลที่แลกได้ พร้อมเวลาหน้าจอคงเหลือในหน้าหลัก' },
];
    ...
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
      ...
      <div className="max-w-6xl mx-auto px-6 flex items-center justify-between">
        ...
        <a href="#" className="flex items-center gap-3 group">
          <img src="/src/assets/Kid_Guard.png" alt="KidGuard" className="w-10 h-10 rounded-xl shadow-lg shadow-sage-500/30 group-hover:shadow-sage-500/50 transition-shadow" />
...
function Features() {
  ...
  return (
    <section id="features" className="relative py-24 sm:py-32 bg-white" ref={ref}>
      {/* Decorative */}
      ...
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className={`text-center mb-16 ${inView ? 'animate-fade-in-up' : 'opacity-0'}`}>
          ...
          <p className="text-sage-500 text-lg max-w-2xl mx-auto">
            KidGuard มาพร้อมฟีเจอร์ครบครันเพื่อให้ผู้ปกครองดูแลบุตรหลานได้อย่างมั่นใจ
          </p>
        </div>
        ...
        <div className="grid sm:grid-cols-2 gap-6">
          {FEATURES.map((f, i) => (
            ...
            >
              {/* Icon */}
              <div className={`w-14 h-14 rounded-2xl bg-gradient-to-br ${f.color} text-white flex items-center justify-center mb-5 shadow-lg shadow-sage-500/20 group-hover:scale-110 transition-transform duration-300`}>
                {f.icon}
              </div>

              <h3 className="text-xl font-bold text-sage-800 mb-3">{f.title}</h3>
...
function HowToUse() {
  ...
  return (
    <section id="how-to-use" className="relative py-24 sm:py-32 bg-gradient-to-br from-sage-50 via-cream-50 to-sage-100" ref={ref}>
      <div className="max-w-6xl mx-auto px-6">
        {/* Header */}
        <div className={`text-center mb-16 ${inView ? 'animate-fade-in-up' : 'opacity-0'}`}>
          ...
          {STEPS.map((s, i) => (
            ...
            >
              ...
              <div className="relative glass rounded-3xl p-8 text-center hover:shadow-xl hover:shadow-sage-200/30 transition-all duration-500 hover:-translate-y-2 group">
                ...
                <h3 className="text-xl font-bold text-sage-800 mb-3">{s.title}</h3>
                <p className="text-sage-500 leading-relaxed text-sm">{s.desc}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className={inView ? 'animate-fade-in-up delay-400' : 'opacity-0'}>
          <h3 className="text-center text-2xl sm:text-3xl font-bold text-sage-900 mb-3">คู่มือการใช้งานละเอียด</h3>
          <p className="text-center text-sage-500 mb-8">เลือกบทบาทเพื่อดูขั้นตอนการตั้งค่า</p>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5 max-w-5xl mx-auto">
            {(activeTab === 'parent' ? PARENT_STEPS : CHILD_STEPS).map((item, i) => (
              <div
                key={`${activeTab}-${i}`}
              ...
              >
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-bold text-sage-400">STEP {String(i + 1).padStart(2, '0')}</span>
                  </div>
                </div>
        ...
        {/* Permission Notice */}
        <div className={`mt-16 glass rounded-2xl p-6 max-w-2xl mx-auto ${inView ? 'animate-fade-in-up delay-600' : 'opacity-0'}`}>
          <div className="flex items-start gap-4">
            <div className="w-10 h-10 rounded-xl bg-amber-100 flex items-center justify-center flex-shrink-0 text-xl">
              ⚠️
            </div>
            <div>
              <h4 className="font-bold text-sage-800 mb-1">การอนุญาตติดตั้ง APK</h4>
              <p className="text-sage-500 text-sm leading-relaxed">
                เนื่องจากแอปยังไม่ได้อยู่บน Play Store ในครั้งแรกที่ติดตั้ง อุปกรณ์จะขออนุญาตให้ "ติดตั้งจากแหล่งที่ไม่รู้จัก" (Unknown Sources)
                ไปที่ <strong>ตั้งค่า → ความปลอดภัย → แหล่งที่ไม่รู้จัก</strong> แล้วเปิดใช้งาน
              </p>
            </div>
          </div>
        </div>
...
function DownloadSection() {
  ...
  return (
    <section id="about-project" className="relative py-24 sm:py-32 bg-gradient-to-br from-sage-800 via-sage-900 to-sage-950 overflow-hidden" ref={ref}>
      {/* Decorative */}
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-6">
        ...
        {/* ─── Project Credits ─── */}
        <div className={`${inView ? 'animate-fade-in-up delay-500' : 'opacity-0'}`}>
          <h3 className="text-center text-2xl sm:text-3xl font-bold text-white mb-2">เกี่ยวกับโปรเจกต์</h3>
          <p className="text-center text-sage-400 mb-12">โปรเจกต์จบการศึกษา — สาขาเทคโนโลยีสารสนเทศและการสื่อสารดิจิทัล</p>
          ...
          <div className="bg-white/5 backdrop-blur-sm rounded-3xl p-8 border border-sage-700/30 hover:border-sage-500/40 transition-colors mb-8">
            <div className="flex items-center gap-3 mb-8 justify-center">
              <div className="w-10 h-10 rounded-xl bg-sage-600/30 flex items-center justify-center text-xl">👥</div>
              <h4 className="text-lg font-bold text-white">ทีมผู้พัฒนา</h4>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-8">
              {[
                { name: 'นางสาว ปัณฑารีย์ ภูมิพลับ', img: '/src/assets/1.jpg' },
                { name: 'นาย เศรษฐพงษ์ ป้อมรุ่ง', img: '/src/assets/2.png' },
                { name: 'นาย อรรถพล ดอกไม้', img: '/src/assets/3.png' },
              ].map((member, i) => (
                <div key={i} className="flex flex-col items-center text-center group">
                  ...
                  <p className="text-sage-200 font-medium text-sm">{member.name}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-8">
            ...
            <div className="bg-white/5 backdrop-blur-sm rounded-3xl p-8 border border-sage-700/30 hover:border-sage-500/40 transition-colors">
              ...
              <ul className="space-y-4">
                <li className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-sage-200 font-medium">ผศ. ไกรมน มณีศิลป์</p>
                ...
                <li className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-sage-200 font-medium">ผศ. พัฒน์นรี จันทราภิรมย์</p>
            ...
            <div className="bg-white/5 backdrop-blur-sm rounded-3xl p-8 border border-sage-700/30 hover:border-sage-500/40 transition-colors">
              ...
              <div className="space-y-3">
                <div>
                  <p className="text-sage-500 text-xs uppercase tracking-wider mb-1">มหาวิทยาลัย</p>
                  <p className="text-sage-200 font-medium">มหาวิทยาลัยเทคโนโลยีราชมงคลธัญบุรี</p>
                </div>
                <div>
                  <p className="text-sage-500 text-xs uppercase tracking-wider mb-1">คณะ</p>
                  <p className="text-sage-200 font-medium">วิทยาศาสตร์และเทคโนโลยี</p>
                </div>
                <div>
                  <p className="text-sage-500 text-xs uppercase tracking-wider mb-1">สาขาวิชา</p>
                  <p className="text-sage-200 font-medium">เทคโนโลยีสารสนเทศและการสื่อสารดิจิทัล</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white/5 backdrop-blur-sm rounded-3xl p-8 border border-sage-700/30 hover:border-sage-500/40 transition-colors md:col-span-2">
              <div className="flex items-center gap-3 mb-6 justify-center">
                <div className="w-10 h-10 rounded-xl bg-sage-600/30 flex items-center justify-center text-xl">⚙️</div>
              ...
              <div className="grid grid-cols-3 gap-4 max-w-md mx-auto">
                ...
                <div className="bg-sage-800/50 rounded-2xl p-5 text-center border border-sage-700/30 hover:border-sage-500/30 hover:bg-sage-700/40 transition-all group">
                  <div className="flex justify-center mb-3">
                    <svg viewBox="0 0 24 24" className="w-10 h-10 text-[#54C5F8] group-hover:scale-110 transition-transform" fill="currentColor">
                ...
                <div className="bg-sage-800/50 rounded-2xl p-5 text-center border border-sage-700/30 hover:border-sage-500/30 hover:bg-sage-700/40 transition-all group">
                  <div className="flex justify-center mb-3">
                    <svg viewBox="0 0 24 24" className="w-10 h-10 text-[#E76F00] group-hover:scale-110 transition-transform" fill="currentColor">
                  ...
                  <p className="text-sage-200 text-sm font-semibold">Java Native</p>
                </div>

                {/* Firebase */}
                <div className="bg-sage-800/50 rounded-2xl p-5 text-center border border-sage-700/30 hover:border-sage-500/30 hover:bg-sage-700/40 transition-all group">
                  <div className="flex justify-center mb-3">
                    <svg viewBox="0 0 24 24" className="w-10 h-10 group-hover:scale-110 transition-transform" fill="none">
                  ...
                  <p className="text-sage-200 text-sm font-semibold">Firebase</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="max-w-6xl mx-auto px-6">
        <div className="flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            <img src="/src/assets/Kid_Guard.png" alt="KidGuard" className="w-8 h-8 rounded-lg" />
